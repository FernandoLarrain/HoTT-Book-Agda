...
Eqz-UMP {𝓤} (A , a₀ , s , i) (B , b₀ , s' , i') (C , c₀ , t , j) (f , f₀ , f-s) (g , g₀ , g-s) =
  _
    ≃⟨ Σ-preserves-≃' _ _ (ϕ , qinv-to-isequiv (ψ , hrefl _ , hrefl _)) (Σ-induction λ h q → ×-preserves-≃
...
-- Eqz-UMP' : (A B C : Alg 𝓤) (f g : Hom A B) → pr₁ (Eqz-UMP A B C f g) ∼ (λ u → (comp C (Eqz A B f g) A (Eqz-map A B f g) u) , (associator C (Eqz A B f g) A B u (Eqz-map A B f g) f ∙ ap (λ - → comp C (Eqz A B f g) B - u) (Eqz-equalizes A B f g) ∙ (associator C (Eqz A B f g) A B u (Eqz-map A B f g) g) ⁻¹))
-- Eqz-UMP' A B C f g u = {!!}
