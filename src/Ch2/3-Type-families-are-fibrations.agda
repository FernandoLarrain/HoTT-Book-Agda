...
apdconst : {A : 𝓤 ̇} (B : 𝓥 ̇) {x y : A} (f : A → B) (p : x ≡ y) → apd f p ≡ transportconst B p (f x) ∙ ap f p
apdconst B f (refl x) = refl (refl (f x))

apdconst' : {A : 𝓤 ̇} (B : 𝓥 ̇) {x y : A} (f : A → B) (p : x ≡ y) → ap f p ≡ transportconst B p (f x) ⁻¹ ∙ apd f p
apdconst' B f (refl x) = refl (refl (f x))
