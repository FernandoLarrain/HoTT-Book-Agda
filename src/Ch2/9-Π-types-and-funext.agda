...
transport-fun : {X : 𝓤 ̇ } {A B : X → 𝓥 ̇ } (x₁ x₂ : X) (p : x₁ ≡ x₂) (f : A x₁ → B x₁) → transport (λ - → A - → B -) p f ∘ transport A p ∼ transport B p ∘ f
transport-fun x₁ .x₁ (refl .x₁) f a = refl (f a)

-- (ii) Book version

transport-fun' : {X : 𝓤 ̇ } {A B : X → 𝓥 ̇ } (x₁ x₂ : X) (p : x₁ ≡ x₂) (f : A x₁ → B x₁) → transport (λ - → A - → B -) p f ∼ transport B p ∘ f ∘ transport A (p ⁻¹)
transport-fun' x₁ .x₁ (refl .x₁) f a = refl (f a)
