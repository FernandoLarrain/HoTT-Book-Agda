...
ua : {A B : 𝓤 ̇} → (A ≃ B) → (A ≡ B)
ua = qinv₁ (isequiv-to-qinv univ)


-- Computation rules for univalence

idtoeqv-β : {A B : 𝓤 ̇} → (f : A ≃ B) → coe (ua f) ∼ pr₁ f
idtoeqv-β f = happly (pr₁ (dpr-≡ (α f)))
  where
  α : (idtoeqv ∘ ua) ∼ id
  α = qinv₂ (isequiv-to-qinv univ)

idtoeqv-β' : {A B : 𝓤 ̇} → (idtoeqv ∘ ua) ∼ 𝑖𝑑 (A ≃ B)
idtoeqv-β' = qinv₂ (isequiv-to-qinv univ)

-- Uniqueness rule for univalence

idtoeqv-η : {A B : 𝓤 ̇} → ua ∘ idtoeqv ∼ 𝑖𝑑 (A ≡ B)
idtoeqv-η = qinv₃ (isequiv-to-qinv univ)
