...
generalized-Axiom-K : (univ : Univalence) (n : Tlevel) (A : 𝓤 ̇) → is (S n) type A ≃ ((a : A) → isContr (pr₁ (Ω^ (to-ℕ n) (A , a))))
generalized-Axiom-K univ ⟨-2⟩ A = ≃-sym (isProp-≃-is-⟨-1⟩-type A) ● isProp-≃-inhabited-to-isContr A
  where open Full-Univalence univ
generalized-Axiom-K univ (S ⟨-2⟩) A = ≃-sym (isSet-≃-is-⟨0⟩-type A) ● (isSet-≃-Axiom-K A ● Axiom-K-≃-Contr-Ω A)
  where open Full-Univalence univ
generalized-Axiom-K univ (S (S n)) A =
  is S (S (S n)) type A
    ≃⟨ Tlevel-in-terms-of-Ω _ _ ⟩
  ((a : A) → is S (S n) type (a ≡ a))
    ≃⟨ Π-preserves-family-≃ (λ a → generalized-Axiom-K univ (S n) (a ≡ a)) ⟩
