...
_is-retract-of_ {𝓤} {𝓥} {𝓦} {𝓣} {A} {B} {X} {Y} g f =
  Σ ρ ꞉ A ◁ X ,
  Σ ρ' ꞉ B ◁ Y ,
  Σ hsec ꞉ f ∘ section ρ ∼ section ρ' ∘ g ,
  Σ hret ꞉ g ∘ retraction ρ ∼ retraction ρ' ∘ f ,
  ((a : A) → ap g (retract-eqn ρ a) ∙ retract-eqn ρ' (g a) ⁻¹ ≡ hret (section ρ a) ∙ ap (retraction ρ') (hsec a))
...
-- Homotopies induce function retracts.

∼-to-retract : {A : 𝓤 ̇} {B : 𝓥 ̇} {f g : A → B} → f ∼ g → g is-retract-of f
∼-to-retract {𝓤} {𝓥} {A} {B} {f} {g} α =
  (id , id , hrefl _) ,
  (id , id , hrefl _) ,
  α ,
  hsym α ,
  (λ a → linv _ ⁻¹ ∙ (α a ⁻¹ ∙ₗ ap-id (α a) ⁻¹))
...
retraction-of-maps-to-fiberwise-retraction {𝓤} {𝓥} {𝓦} {𝓣} {A} {B} {X} {Y} {g} {f} ((r , s , R) , (r' , s' , R') , L , K , H) b = (ψ b) , ((ϕ b) , (retraction-equation b))
  ...
  private
    ...
    {- Note : the full equivalence seems to require FunExt, but we can prove the necessary direction without it. -}

    equivalence : ⦃ fe : FunExt ⦄ → ((b : B) → ψ b ∘ ϕ b ∼ id) ≃ ((a : A) → ψ (g a) (ϕ (g a) (a , refl (g a))) ≡ (a , refl (g a)))
  ...
  suffices : ((a : A) → ψ (g a) (ϕ (g a) (a , refl (g a))) ≡ (a , refl (g a))) → ((b : B) → ψ b ∘ ϕ b ∼ id)
  suffices antecedent .(g a) (a , refl .(g a)) = antecedent a
  ...
  retraction-equation = suffices (λ a → dpair-≡ ((R a) , (
...
retract-of-ContrMap-is-ContrMap g f c ρ y = retract-of-Contr-is-Contr (retraction-of-maps-to-fiberwise-retraction ρ y) (c _)
...
total-fib-≃ {𝓤} {𝓥} {𝓦} {A} {P} {Q} f x v =
  ...
  (Σ a ꞉ A , Σ u ꞉ P a , (a , f a u) ≡ (x , v))
    ≃⟨ Σ-preserves-family-≃ (λ a → Σ-preserves-family-≃ λ u → Σ-≡-≃) ⟩
...
module fiberwise-≃-iff-total-≃ {A : 𝓤 ̇} {P : A → 𝓥 ̇} {Q : A → 𝓦 ̇} (f : Π (λ x → P x → Q x)) where
  ...
  ContrMap : ((x : A) → isContrMap (f x)) ⇔ isContrMap (total f)
  ContrMap = sufficiency , necessity where
  ...
  Hae = sufficiency , necessity where
    ...
    sufficiency f-is-hae = isContrMap-to-ishae (total f) (pr₁ ContrMap (λ x → ishae-to-isContrMap (f x) (f-is-hae x)))
  ...
  Biinv = sufficiency , necessity where
    ...
    necessity t-is-hae x = isContrMap-to-biinv (f x) (pr₂ ContrMap (biinv-to-isContrMap (total f) t-is-hae) x)


-- Sufficient condition for embedding.

embedding-criterion : {A : 𝓤 ̇} {B : 𝓥 ̇} (f : A → B) → ((x x' : A) → (f x ≡ f x') ≃ (x ≡ x')) → isEmbedding f
